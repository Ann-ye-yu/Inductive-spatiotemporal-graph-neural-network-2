...
def train_multiple_epochs(train_dataset,
    ...
    """
    :param train_dataset: MyDynamicDataset(900188)
    :param test_dataset: MyDataset(100021)
    :param model: IGMC
    :param epochs: 40
    :param batch_size: 8
    :param lr: 0.001
    :param lr_decay_factor: 0.1
    :param lr_decay_step_size: 20
    :param weight_decay: 0
    :param test_freq:1
    :param logger:None
    :param continue_from: None
    :param res_dir: /home/zhaojinye/papers_codes/IGMC/results_d_30b_32dim/ml_1m_mnhp100_testmode
    :return:
    """
    rmses = []
...
def train(model, optimizer, loader, device, regression=False,
    ...
    i = 1
    '''
    这里每个data的长度是5，每个图里面是30 batchsize
    '''
    for data in pbar:
        # print(f"第{i}batch_size:\n{data}")
        # print("***************")
        # print(len(data))
        # for k in range(len(data)):   # k = 5->子图个数
        #     for j in range(data[k].num_graphs):  # j :0-30
        #         print(data[k][j])
        i += 1
        optimizer.zero_grad()
        subgraph_data_list = []
        ...
        for i in range(len(data)):
            subgraph_data = data[i].to(device)
            # print('\n')
            # print(subgraph_data)
            subgraph_data_list.append(subgraph_data)
        out = model(subgraph_data_list)
        if regression:
            loss = F.mse_loss(out, data[0].y.view(-1))
        else:
            loss = F.nll_loss(out, data[0].y.view(-1))
        if show_progress:
            pbar.set_description('Epoch {}, batch loss: {}'.format(epoch, loss.item()))
        loss.backward()
        total_loss += loss.item() * num_graphs(data[0])
        optimizer.step()
        # 内存释放
        del data, out, loss, subgraph_data, subgraph_data_list
...
def eval_loss(model, loader, device, regression=False, show_progress=False):
    ...
    for data in pbar:
        ...
        with torch.no_grad():
            out = model(subgraph_list)
        if regression:
            loss += F.mse_loss(out, data[0].y.view(-1), reduction='sum').item()
        else:
            loss += F.nll_loss(out, data[0].y.view(-1), reduction='sum').item()
        del data, out, subgraph_data, subgraph_list
        torch.cuda.empty_cache()
    ...
    for i, checkpoint in enumerate(checkpoints):
        ...
        for data in pbar:
            ...
            for j in range(len(data)):
                subgraph_data = data[j].to(device)
                subgraph_list.append(subgraph_data)
            if i == 0:
                ys.append(data[0].y.view(-1))
            with torch.no_grad():
                out = model(subgraph_list)
                outs.append(out)
        if i == 0:
